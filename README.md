...
This repository exists to show the actual progression, including regressions. **Do not choose a checkpoint just because one metric looks better.** All five included adapters are experimental; our frozen selection rule promoted none from this latest series. They are MLX-VLM-specific adapters, not a standalone model and not standard PEFT adapters. Neither this GitHub repository nor the [matching Hugging Face archive](https://huggingface.co/OmTheLast/jee-solver-experimental-mlx-adapters) hosts an interactive inference endpoint. See `load_mlx.py` for local loading on a compatible Apple Silicon Mac and [EXPERIMENT_HISTORY.md](EXPERIMENT_HISTORY.md) for the preceding studies.
